...
// ==================== ОТПРАВКА ЗАКАЗА (с обновлением остатков) ====================
async function submitOrder() {
    ...
    try {
        ...
        // Сохраняем обновлённые товары в localStorage (для синхронизации между страницами)
        localStorage.setItem("s-l-e-n-g-products", JSON.stringify(window.products));
...
// 5. Адрес — разрешаем пробелы, только убираем лишние множественные
const addressInput = document.getElementById("address");
if (addressInput) {
    addressInput.addEventListener("input", (e) => {
        let value = e.target.value;
        // Заменяем множественные пробелы на один, но не удаляем их полностью
        value = value.replace(/\s+/g, " ");
        // Убираем пробел в начале, если пользователь случайно поставил
